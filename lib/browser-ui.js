...
function createBrowserApi() {
    return {
        onDeviceRefreshEnd: function (nodeId) {
            ...
            panel.innerHTML = [
                '<style>.ct-page{max-width:980px;padding:12px}.ct-page section{border:1px solid #bbb;border-radius:6px;margin:12px 0;padding:12px}.ct-page label{display:block;margin:6px 0}.ct-page input[type=text],.ct-page input:not([type]),.ct-page textarea,.ct-page select{box-sizing:border-box;max-width:100%;width:520px}.ct-page textarea{min-height:68px}.ct-native-row{display:grid;grid-template-columns:minmax(280px,2fr) repeat(3,minmax(90px,1fr));gap:8px;padding:5px;border-bottom:1px solid #ddd}.ct-on{color:#16733a;font-weight:bold}.ct-off{color:#666}.ct-inherited{color:#9a6500}.ct-table{border-collapse:collapse;margin:8px 0;width:100%}.ct-table td,.ct-table th{border:1px solid #bbb;padding:5px;text-align:left}#ct-preview-title{font-weight:bold;margin-top:8px}#ct-preview-message{white-space:pre-wrap}.ct-page button{margin:8px 8px 8px 0}</style>',
                '<div class="ct-page">',
                ...
                '<p>Version 0.1.0 reports Desktop, Terminal and Files relay sessions. It does not add session blocking, prompt enforcement or mobile-specific UI.</p>',
                '<button id="ct-save" data-ct-edit="1" type="button">Save</button>',
                ...
                '<div id="ct-status" role="status" aria-live="polite"></div>',
                '</section></div>'
            ].join("");
